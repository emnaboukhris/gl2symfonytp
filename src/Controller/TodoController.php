...
class TodoController extends AbstractController
{
    ...
    public function addTodo($name, $content, SessionInterface $session) {
        ...
        /**
         * @Route("/mod/{name}/{newcontent}", name="modifierTodo")
         */
        public function modifierTodo($name, SessionInterface $session,$newcontent) {
            ...
            if (!$session->has('todos')) {
                ...
                if (!isset($todos[$name])) {
                    ...
                    $session->set('todos', $todos);
                    $this->addFlash('success', "Le todo $name a été modifier avec succès");
                }
            }
            return $this->redirectToRoute('todo');
        }
    ...
    public function suppTodo($name, SessionInterface $session) {
        ...
        if (!$session->has('todos')) {
            ...
            if (!isset($todos[$name])) {
                ...
                unset($todos[$name]) ;
                $session->set('todos', $todos);
                $this->addFlash('success', "Le todo $name a été supprime avec succès");
            }
        }
        return $this->redirectToRoute('todo');
    }
